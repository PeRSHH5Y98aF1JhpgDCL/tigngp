loadedUpgs=() => {}
//github is bad and dumb and bad so i have to refresh this
window.Layer=class {
  ...
  update(diff) {
    ...
    while (this.state && this.dims.length > 5) {
    ...
    if (this.state&&this.dims[1]) this.dims[0].amount.addBy(secretFormula(this.dims[1].amount,this.dims[1].dim))
...
window.Dimension=class Dimension extends hasCache {
  ...
  get cost() {
	...
	temp=temp.mul(timp.pow(timp.pow(timp.pow(0.5).div(3))))//get  sniped
	}
	  let teemp
